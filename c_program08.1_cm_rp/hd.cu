...
extern __constant__ float rdx_c;

extern __global__ void horizontal_diffusion_gpu(int ids, int ide, int jds, int jde,
...
int horizontal_diffusion_host_(int *ids, int *ide, int *jds, int *jde, int *kds, int *kde,
	...
	//cudaMemcpyToSymbol ((void*)&rdx_c, (void*)&rdx, sizeof(float) );
