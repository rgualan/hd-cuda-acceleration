...
//__constant__ float rdx_;
//__constant__ float rdy_;
//__device__ __constant__ float rdx_;
//__device__ __constant__ float rdy_;
__constant__ float rdx_c;
...
//			}
		...
		for( k=kts-1; k<=ktf-1; k++) {
			...
			mkrdxm=( msftx_s[S2(ti-1,tj)]/msfty_s[S2(ti-1,tj)] ) * mu_s[S2(ti-1,tj)] * xkmhd[P3(ti-1,k,tj)] * rdx;
			mkrdxp=( msftx_s[S2(ti,tj)]/msfty_s[S2(ti,tj)])*mu_s[S2(ti,tj)]*xkmhd[P3(ti,k,tj)]*rdx;
			mrdx=msfux_s[S2(ti,tj)]*msfuy_s[S2(ti,tj)]*rdx;

			mkrdym=( (msfuy_s[S2(ti,tj)]+msfuy_s[S2(ti,tj-1)])/(msfux_s[S2(ti,tj)]+msfux_s[S2(ti,tj-1)]) )*
					0.25*(mu_s[S2(ti,tj)]+mu_s[S2(ti,tj-1)]+mu_s[S2(ti-1,tj-1)]+mu_s[S2(ti-1,tj)])*
					0.25*(xkmhd[P3(ti,k,tj)]+xkmhd[P3(ti,k,tj-1)]+xkmhd[P3(ti-1,k,tj-1)]+xkmhd[P3(ti-1,k,tj)])*rdy;
			mkrdyp=( (msfuy_s[S2(ti,tj)]+msfuy_s[S2(ti,tj+1)])/(msfux_s[S2(ti,tj)]+msfux_s[S2(ti,tj+1)]) )*
					0.25*(mu_s[S2(ti,tj)]+mu_s[S2(ti,tj+1)]+mu_s[S2(ti-1,tj+1)]+mu_s[S2(ti-1,tj)])*
					0.25*(xkmhd[P3(ti,k,tj)]+xkmhd[P3(ti,k,tj+1)]+xkmhd[P3(ti-1,k,tj+1)]+xkmhd[P3(ti-1,k,tj)])*rdy;
