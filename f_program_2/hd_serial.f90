   ...
   IF (name .EQ. 'u') THEN
      ...
      DO i = i_start, i_end
         ...
         mkrdym=( (msfuy(i,j)+msfuy(i,j-1))/(msfux(i,j)+msfux(i,j-1)) )* &
                0.25*(mu(i,j)+mu(i-1,j-1)+mu(i,j-1)+mu(i-1,j))* &
                0.25*(xkmhd(i,k,j)+xkmhd(i-1,k,j-1)+xkmhd(i,k,j-1)+xkmhd(i-1,k,j))*rdy
         mkrdyp=( (msfuy(i,j)+msfuy(i,j+1))/(msfux(i,j)+msfux(i,j+1)) )* &
                0.25*(mu(i-1,j)+mu(i,j)+mu(i-1,j+1)+mu(i,j+1))* &
                0.25*(xkmhd(i-1,k,j)+xkmhd(i,k,j)+xkmhd(i-1,k,j+1)+xkmhd(i,k,j+1))*rdy
