...
__global__ void horizontal_diffusion_gpu(const int ids, const int ide,
	...
	// Shared memory
	// For this case, i.e. ussing 3D block decomposition
	// it did not work to move 2d arrays to shared memory
	// only moving 3d array worked
	__shared__ float xkmhd_s[((XXX + 2) * (YYY + 2))]; // Only 2D variable, because a tile is 2D
...
//  if (name == 'u') {
	...
	xkmhd_s[S2(ti-1,tj-1)] = xkmhd[P3(ti-1,k,tj-1)];
	xkmhd_s[S2(ti+1,tj-1)] = xkmhd[P3(ti+1,k,tj-1)];
	xkmhd_s[S2(ti-1,tj+1)] = xkmhd[P3(ti-1,k,tj+1)];
	xkmhd_s[S2(ti+1,tj+1)] = xkmhd[P3(ti+1,k,tj+1)];
	__syncthreads();
	...
	mkrdym = ((msfuy[P2(ti,tj)] + msfuy[P2(ti,tj-1)])
			...
			* (mu[P2(ti,tj)] + mu[P2(ti,tj-1)] + mu[P2(ti-1,tj-1)]
					+ mu[P2(ti-1,tj)]) * 0.25
			* (xkmhd_s[S2(ti,tj)] + xkmhd_s[S2(ti,tj-1)]
					+ xkmhd_s[S2(ti-1,tj-1)] + xkmhd_s[S2(ti-1,tj)]) * rdy;
