...
void horizontal_diffusion_c_(int *ids, int *ide, int *jds, int *jde, int *kds,
	...
	if (*name == 'u') {
		...
		printf("Iterations to parallelize (j dim): %d\n",
				(J(j_end) - J(j_start) + 1));
...
//#pragma omp parallel default(shared) private(j,k,i)
#pragma omp parallel default(shared) private(nt, id, chunk, j,k,i, mkrdxm, mkrdxp, mrdx, mkrdym, mkrdyp, mrdy)
		{
			nt = omp_get_num_threads();
			id = omp_get_thread_num();
			//chunk = (J(j_end) - J(j_start) + 1) / nt;
			//q = x/y + (x % y != 0);
			chunk = (J(j_end) - J(j_start) + 1) / nt
					+ ((J(j_end) - J(j_start) + 1) % nt != 0);

			printf("Thread %d running... (%d)\n", id, chunk);

			for (j = J(j_start) + id * chunk;
					j <= J(j_start) + (id + 1) * chunk - 1 && j <= J(j_end);
					j++) {
