...
void horizontal_diffusion_c_(int *ids, int *ide, int *jds, int *jde, int *kds,
	...
	if (*name == 'u') {
		...
		int bs = jx / omp_get_num_threads();
		int m;

		printf ("Chunk size",  bs);
...
//#pragma omp parallel default(shared) private(j,k,i)
#pragma omp parallel default(shared) private(j,k,i, mkrdxm, mkrdxp, mrdx, mkrdym, mkrdyp, mrdy)
		{
